...
class Animal {
    ...
    display() {
      let bt = document.querySelector(".toggelBt") as HTMLElement;
      if (
        this.vaccine ==
  ...
  <div class="card-body p-0">
    ...
    <button type="button" class="btn btn-${this.color} toggelBt w-100  shadow p-2 fw-bolder fs-3 rounded-pill " >${this.vaccine}</button>
  </div>
  </div>`;
    }
  }
  ...
  class cats extends Animal {
    ...
    display() {
      let bt = document.querySelector(".toggelBt") as HTMLElement;
      if (
        (this.vaccine = "vaccine" + "   " + `<i class="fa-solid fa-syringe"></i>`)
      ) {
        this.color = "success";
      } else {
        this.color = "danger";
  ...
  function sortAge(){
    ...
    let btns = document.getElementsByClassName("toggelBt")as any;
  ...
  (document.getElementById("toggle")as HTMLElement).addEventListener("click",sortAge)
